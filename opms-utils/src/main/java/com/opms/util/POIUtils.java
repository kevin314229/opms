...
public class POIUtils {
	 ...
	 * @throws IOException
	 */
	private static Workbook openWorkbook(InputStream in, String filename) throws IOException {
		Workbook wb = null;
	...
	public static List<List<String>> getExcelData(MultipartFile file, String fileName, int sheetIndex) throws Exception {
		...
		Sheet sheet = wb.getSheetAt(sheetIndex);// 切换工作薄
		Row row = null;
		Cell cell = null;
	...
	private static boolean curRowInsideNull(Row row, int totalCells) {
		...
		for (int i = 0; i < totalCells; i++) {
			row.getCell(i, Row.RETURN_BLANK_AS_NULL);
				...
				for (int n = begin; n < end; n++) {
					...
					for (int x = 0; x < headers.size(); x++) {
						...
						if (value instanceof Date) {
							...
							/*
							 * if(((String)value).trim().length()>0 && isNumeric((String)value)){ cell.setCellType(Cell.CELL_TYPE_STRING); cell.setCellValue((String)value); }else{ }
							 */
	 ...
	 */
	@SuppressWarnings("rawtypes")
	public static void exportToExcel(HttpServletResponse response, String fileName, List objs, Class clazz, String sheetName, int pageSize) {
	 ...
	 * @return
	 */
	private static CellStyle setCellStyle(HSSFWorkbook workbook, String position) {
