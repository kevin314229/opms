...
<script type="text/javascript">
	...
	$(function() {
		grid = window.opms.ui.grid({
			...
			jsonUrl : '${ctx}/background/resource/resources.html',
			checkbox : true,
			usePage : false,
			records : "resourceTree",
			treeGrid : {
				tree : true,
				name : 'name'
			}
		...
		$("#rowline").click("click", function() {//update排序
			...
			grid.setOptions({
				jsonUrl : rootPath + '/background/resource/sortUpdate.html',
				data : data.join("&")
			});
		});
		...
		$("#add").click("click", function() {//绑定查询按扭
			dialog = parent.$.ligerDialog.open({
				width : 315,
				height : 300,
				url : rootPath + '/background/resource/addUI.html',
				title : "添加资源",
				isHidden : false
		...
		$("#editView").click(
				...
				function() {//绑定查询按扭
					...
					dialog = parent.$.ligerDialog.open({
						...
						height : 310,
						url : rootPath
								+ '/background/resource/editUI.html?resourceId='
								+ cbox,
						title : "修改资源",
						isHidden : false
					});
		...
		$("#deleteView")
				.click(
						...
						function() {//绑定查询按扭
							...
							if(u!=""){
								 parent.$.ligerDialog.warn("原数据禁止删除！你的IP已经记录！有问题请联系管理员");
												...
												if (confirm) {
													$
															.ajax({
																...
																url : rootPath
																		+ '/background/resource/deleteById.html', //要访问的后台地址
																data : {
																	ids : cbox
																			.join(",")
																}, //要发送的数据
